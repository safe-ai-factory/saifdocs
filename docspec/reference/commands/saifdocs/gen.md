...
Full compile: read `docspec/`, write `docspec/.manifest.json`, then emit a saifctl feature tree under `<project>/saifctl/features/saifdocs-<timestamp>/` with one phase per file-to-generate plus an `audit` critic. Phases are ordered: references → concepts → how-to's → tutorials → landing-pages.
